...
const OtherSetting = () => {
  ...
  const openGitHubRelease = () => {
    window.location =
      'https://github.com/cubeofcube-dev/one-api/releases/latest';
  };

  const checkUpdate = async () => {
    const res = await API.get(
      'https://api.github.com/repos/cubeofcube-dev/one-api/releases/latest'
    );
          ...
          <Form.Group widths='equal'>
            <Form.Input
              label={<label>主题名称（<Link
                to='https://github.com/cubeofcube-dev/one-api/blob/main/web/README.md'>当前可用主题</Link>）</label>}
              placeholder='请输入主题名称'
              value={inputs.Theme}
              name='Theme'
              onChange={handleInputChange}

...
describe("timezone specific handling", () => {
	it("handles Toronto standard time correctly", () => {
		...
		expect(formatTimestamp(timestamp, { timeZone: TORONTO_TZ })).toBe(
			"2024-11-12 13:21:00",
		);
		expect(toDateTimeLocal(timestamp, { timeZone: TORONTO_TZ })).toBe(
			"2024-11-12T13:21",
		);
		expect(
			fromDateTimeLocal("2024-11-12T13:21", { timeZone: TORONTO_TZ }),
		).toBe(timestamp);
	});
	...
	it("handles Toronto daylight time correctly", () => {
		...
		expect(
			fromDateTimeLocal("2024-06-12T13:21", { timeZone: TORONTO_TZ }),
		).toBe(timestamp);
	});

...
export function logEditPageLayout(label = "EditPage") {
	...
	const logOnce = () => {
		try {
			...
			const footerBottom = footerRect
				? footerRect.bottom + window.scrollY
				: null;
			const trailingAfterFooter = footerBottom
				? docBottom - footerBottom
				: null;
			...
			const culpritInfo = culprit
				? {
						...
						rect: (culprit.getBoundingClientRect() as DOMRect).toJSON
							? (culprit.getBoundingClientRect() as DOMRect).toJSON()
			...
			// biome-ignore lint/suspicious/noExplicitAny: debug helper

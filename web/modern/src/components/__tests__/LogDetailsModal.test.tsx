...
describe("LogDetailsModal", () => {
	...
	it("renders a detailed view that mirrors the logs table fields", async () => {
		...
		expect(
			screen.getAllByText(formatTimestamp(log.created_at)).length,
		).toBeGreaterThan(0);
		...
		expect(
			screen.getByText(formatLatencyForTest(log.elapsed_time)),
		).toBeInTheDocument();

		const promptInput = screen
			.getByText(/prompt tokens \(input\)/i)
			.closest("div");
		expect(promptInput).toHaveTextContent("1200");

		const completionOutput = screen
			.getByText(/completion tokens \(output\)/i)
			.closest("div");
		expect(completionOutput).toHaveTextContent("800");

		const cacheWrite5m = screen
			.getByText(/cache write 5m tokens/i)
			.closest("div");
		expect(cacheWrite5m).toHaveTextContent("100");

		const cacheWrite1h = screen
			.getByText(/cache write 1h tokens/i)
			.closest("div");
		...
		const totalCachedTokens = screen
			.getByText(/total cached tokens/i)
			.closest("div");
		...
		expect(
			screen.getByText(/tracing data is not available/i),
		).toBeInTheDocument();

...
import { type User, useAuthStore } from "@/lib/stores/auth";
...
describe("Header logout UX", () => {
	...
	beforeEach(() => {
		...
		(api.get as unknown as Mock).mockResolvedValue({ data: { success: true } });
	});
	...
	it("offers logout inside the mobile navigation drawer", async () => {
		...
		const drawerLogoutButton = await screen.findByRole("button", {
			name: /logout/i,
		});

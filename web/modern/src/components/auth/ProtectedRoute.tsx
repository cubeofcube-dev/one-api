...
import { type AuthState, useAuthStore } from "@/lib/stores/auth";

interface TestableAuthState extends AuthState {
	validateSession?: () => Promise<boolean>;
}

export function ProtectedRoute() {
	// Read the whole store to allow tests to inject extra fields like validateSession
	const store = useAuthStore() as unknown as TestableAuthState;
	const { user } = store;

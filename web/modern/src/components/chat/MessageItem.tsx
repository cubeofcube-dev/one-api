...
export function MessageItem({
	...
	const handleSaveEdit = (newContent: string | MessageContentPart[]) => {
		...
		const contentStructureChanged =
			JSON.stringify(newContent) !== JSON.stringify(message.content);
					...
					<div className="max-w-3xl space-y-3">
						...
						<div className="flex items-center justify-between mb-2">
							...
							<UserMessageActions
								...
								onDeleteMessage={
									onDeleteMessage ? handleDeleteMessage : undefined
								}
							/>
							...
							<div className="flex items-center justify-between mb-2">
								<div className="flex items-center gap-2">
									...
									<span className="font-medium text-sm text-indigo-800 dark:text-indigo-200">
										{t("playground.roles.system_message")}
									</span>
								</div>
									...
									<DropdownMenu>
										<DropdownMenuTrigger asChild>
											<Button
												variant="ghost"
												size="sm"
												className="h-6 w-6 p-0 text-indigo-600/70 dark:text-indigo-400/70 hover:text-indigo-600 dark:hover:text-indigo-400"
											>
					...
					<div className="max-w-3xl space-y-3">
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button
											variant="ghost"
											size="sm"
											className="h-6 w-6 p-0 text-red-500/70 hover:text-red-500"
										>
											<MoreHorizontal className="h-3 w-3" />
										</Button>
									</DropdownMenuTrigger>
						...
						<div className="rounded-lg px-4 py-3 bg-red-50 border border-red-200 text-red-800 dark:bg-red-900/20 dark:border-red-800 dark:text-red-300 shadow-md">
							<div className="whitespace-pre-wrap">
								{getMessageStringContent(message.content)}
							</div>
						</div>
					</div>
				</div>

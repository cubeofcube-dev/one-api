...
import type { MessageContentPart } from "@/lib/utils";

interface EditMessageDialogProps {
	isOpen: boolean;
	onClose: () => void;
	onSave: (newContent: string | MessageContentPart[]) => void;
	currentContent: string;
	originalContent: string | MessageContentPart[]; // The full message content including attachments
...
export function EditMessageDialog({
	...
	useEffect(() => {
		...
		if (Array.isArray(originalContent)) {
			const attachments = originalContent.filter(
	...
	useEffect(() => {
		...
		const attachmentsChanged =
			...
			JSON.stringify(editedAttachments) !==
				JSON.stringify(
					originalContent.filter((item) => item?.type === "image_url"),
				);
	...
	const handleDeleteAttachment = (attachmentIndex: number) => {
		const updatedAttachments = editedAttachments.filter(
					...
					<div className="space-y-3">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
							<Label
								...
								className={`text-sm font-semibold ${getRoleColor()} flex-shrink-0`}
							>
								{t("playground.edit.content_label", {
					...
					{editedAttachments.length > 0 && (
						<div className="space-y-2">
							<Label className="text-sm font-medium flex items-center gap-2">
								...
								{t("playground.edit.attachments_label", {
									count: editedAttachments.length,
								})}
							</Label>
							<div className="max-h-[120px] sm:max-h-[180px] overflow-y-auto">
								<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 p-3 bg-muted/30 rounded-lg">
									{editedAttachments.map((attachment, index) => (
										<Card
											key={attachment.image_url?.url || index}

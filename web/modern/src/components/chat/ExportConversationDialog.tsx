					...
					<DialogDescription className="space-y-2">
						...
						{messages.length > 0 && (
							<div className="text-sm text-muted-foreground">
								{t("playground.export.ready_to_export", {
									count: messages.length,
									plural: messages.length === 1 ? "" : "s",
								})}
								{selectedModel &&
									t("playground.export.from_model", { model: selectedModel })}
							</div>
						)}
					</DialogDescription>
				</DialogHeader>
						...
						>
							...
							<div className="text-center">
								<div className="font-medium">
									{t("playground.export.button_label")}
								</div>
								<div className="text-xs text-muted-foreground">
									{t("playground.export.button_sublabel")}
								</div>
							</div>
						</Button>
					</div>
					...
					{isExporting && (
						<div className="flex items-center justify-center gap-2 py-2">
							...
							<span className="text-sm text-muted-foreground">
								{t("playground.export.generating")}
							</span>
						</div>
					)}
				</div>

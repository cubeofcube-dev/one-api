import { AlertCircle, FileX, ImageIcon, X } from "lucide-react";
...
export function ImageAttachmentComponent({
			...
			return new Promise((resolve, reject) => {
				...
				const timeout = setTimeout(
					() => {
						reject(
							new Error(
								`Image compression timeout after ${isMobile ? "15" : "30"} seconds`,
							),
						);
					},
				...
				img.onload = () => {
					try {
						...
						const mobileMaxWidth = isMobile
							...
							: maxWidth;
						const mobileMaxHeight = isMobile
							...
							async (blob) => {
								if (blob) {
									try {
										...
										reject(
											new Error(
												`Base64 conversion failed: ${conversionError instanceof Error ? conversionError.message : "Unknown error"}`,
											),
										);
									}
								} else {
									...
									reject(
										new Error(
											"Canvas toBlob failed - browser may be out of memory",
										),
									);
								}
							},
			...
			try {
				...
				img.onload = () => {
					try {
						...
						reject(
							new Error(
								`Canvas conversion failed: ${canvasError instanceof Error ? canvasError.message : "Unknown error"}`,
							),
						);
					}
				};
	...
	const processImagesInParallel = useCallback(
		async (files: File[]): Promise<ImageAttachment[]> => {
			...
			const concurrencyLimit = isMobile
				? 1
				: Math.min(3, navigator.hardwareConcurrency || 2);
	...
	return (
		<div className="space-y-2 sm:space-y-3">
				...
				<div className="space-y-2">
					...
					<div className="space-y-2 max-h-32 overflow-y-auto">
						{validationErrors.map((error, index) => (
							<Card
								// biome-ignore lint/suspicious/noArrayIndexKey: error list is transient
								key={index}
								...
								<CardContent className="p-3">
									<div className="flex items-start gap-2">
										...
										<div className="flex-1 min-w-0">
											<div
												className="text-sm font-medium text-red-800 dark:text-red-200 truncate"
					...
					<div className="text-xs text-muted-foreground">
						💡 Tip: You can dismiss these errors by clicking the × button on
						each card.
					</div>
				</div>
			)}
			...
			{/* Info text */}
			<div className="text-xs text-muted-foreground">
				Supports: JPG, PNG, GIF, WebP • Max 5MB per image • Up to {maxImages}{" "}
				images
			</div>
		</div>
	);

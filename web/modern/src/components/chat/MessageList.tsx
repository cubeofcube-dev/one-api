...
export function MessageList({
	...
	useEffect(() => {
		...
		scrollToBottom();
	}, [focusModeEnabled, isStreaming, scrollToBottom]);
		...
		<ScrollArea ref={scrollAreaRef} className="h-full p-4">
			<div className="space-y-4">
				{messages.length === 0 && (
					<div className="text-center text-muted-foreground py-12">
						...
						<p className="text-lg font-medium">
							{t("playground.chat.empty_state.title")}
						</p>
						<p className="text-sm mt-2">
							{t("playground.chat.empty_state.description")}
						</p>
					</div>
				)}
				...
				{messages.map((message, index) => (
					<div
						// biome-ignore lint/suspicious/noArrayIndexKey: messages list is append-only and index is stable enough for display
						key={index}
						className={`${
							message.role === "user"
								? "flex justify-end"

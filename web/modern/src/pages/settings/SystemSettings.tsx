...
const OPTION_GROUPS: OptionGroup[] = [
	...
	},
	{
		...
		description:
			"Automatically react to upstream channel health and retry behavior.",
		keys: [
...
const OPTION_GROUP_KEY_SET = new Set(
	OPTION_GROUPS.flatMap((group) => group.keys),
);
...
export function SystemSettings() {
	...
	const descriptions = useMemo<Record<string, string>>(
		() => ({
			...
			PasswordLoginEnabled: t(
				"system_settings.descriptions.PasswordLoginEnabled",
			),
			PasswordRegisterEnabled: t(
				"system_settings.descriptions.PasswordRegisterEnabled",
			),
			RegisterEnabled: t("system_settings.descriptions.RegisterEnabled"),
			EmailVerificationEnabled: t(
				"system_settings.descriptions.EmailVerificationEnabled",
			),
			EmailDomainRestrictionEnabled: t(
				"system_settings.descriptions.EmailDomainRestrictionEnabled",
			),
			EmailDomainWhitelist: t(
				"system_settings.descriptions.EmailDomainWhitelist",
			),
			...
			OidcAuthorizationEndpoint: t(
				"system_settings.descriptions.OidcAuthorizationEndpoint",
			),
			OidcTokenEndpoint: t("system_settings.descriptions.OidcTokenEndpoint"),
			OidcUserinfoEndpoint: t(
				"system_settings.descriptions.OidcUserinfoEndpoint",
			),
			...
			WeChatServerAddress: t(
				"system_settings.descriptions.WeChatServerAddress",
			),
			WeChatServerToken: t("system_settings.descriptions.WeChatServerToken"),
			WeChatAccountQRCodeImageURL: t(
				"system_settings.descriptions.WeChatAccountQRCodeImageURL",
			),

			// Anti-bot / Security
			TurnstileCheckEnabled: t(
				"system_settings.descriptions.TurnstileCheckEnabled",
			),
			...
			QuotaRemindThreshold: t(
				"system_settings.descriptions.QuotaRemindThreshold",
			),
			...
			DisplayInCurrencyEnabled: t(
				"system_settings.descriptions.DisplayInCurrencyEnabled",
			),
			DisplayTokenStatEnabled: t(
				"system_settings.descriptions.DisplayTokenStatEnabled",
			),
			ApproximateTokenEnabled: t(
				"system_settings.descriptions.ApproximateTokenEnabled",
			),

			// Channels & Reliability
			AutomaticDisableChannelEnabled: t(
				"system_settings.descriptions.AutomaticDisableChannelEnabled",
			),
			AutomaticEnableChannelEnabled: t(
				"system_settings.descriptions.AutomaticEnableChannelEnabled",
			),
			ChannelDisableThreshold: t(
				"system_settings.descriptions.ChannelDisableThreshold",
			),
			...
			MessagePusherAddress: t(
				"system_settings.descriptions.MessagePusherAddress",
			),
						...
						<div className="space-y-10">
							...
							{uncategorizedOptions.length > 0 && (
								<section className="space-y-4">
									<div className="space-y-1">
										<h3 className="text-lg font-semibold leading-6">
											{t("system_settings.groups.other.title")}
										</h3>
										<p className="text-sm text-muted-foreground">
											{t("system_settings.groups.other.description")}
										</p>
									</div>
									<div className="grid grid-cols-1 gap-4 md:grid-cols-2">

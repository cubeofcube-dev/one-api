...
describe("LoginPage", () => {
	...
	it("shows TOTP input when TOTP is required", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByPlaceholderText(/6-digit totp code/i),
			).toBeInTheDocument();
		});
		...
		expect(
			screen.getByRole("button", { name: /verify totp/i }),
		).toBeInTheDocument();
	});
	...
	it("successfully logs in with valid TOTP code", async () => {
		...
		fireEvent.change(screen.getByPlaceholderText(/6-digit totp code/i), {
			target: { value: "123456" },
		});
	...
	it("shows back to login button in TOTP mode", async () => {
		...
		expect(
			screen.queryByPlaceholderText(/6-digit totp code/i),

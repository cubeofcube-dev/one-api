...
import { barColor, chartConfig, getDisplayInCurrency, getQuotaPerUnit } from '../services/chartConfig'
...
export const useDashboardData = ({
  ...
  const loadStats = useCallback(async (rangeOverride?: { from: string; to: string }) => {
  ...
  const computeStackedSeries = useCallback(<T extends BaseMetricRow>(
    rowsSource: T[],
    daysList: string[],
    ...
    for (const row of rowsSource) {
      ...
      dayToValues[row.day][label] = (dayToValues[row.day][label] || 0) + value
    }
  ...
  const { uniqueKeys: modelKeys, stackedData: modelStackedData } = useMemo(
    ...
    [rows, xAxisDays, statisticsMetric, t, computeStackedSeries]
  )
  ...
  const { uniqueKeys: userKeys, stackedData: userStackedData } = useMemo(
    ...
    [userRows, xAxisDays, statisticsMetric, t, computeStackedSeries]
  )
  ...
  const { uniqueKeys: tokenKeys, stackedData: tokenStackedData } = useMemo(
    ...
    [tokenRows, xAxisDays, statisticsMetric, t, computeStackedSeries]
  )
    ...
    return ({ active, payload, label }: TooltipProps<number, string>) => {
      if (active && payload && payload.length) {
        const filtered = payload
          .filter((entry) => entry && entry.value && typeof entry.value === 'number' && entry.value > 0)

  ...
  <div className="mb-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
      <Card title={t('dashboard.cards.total_requests')} value={formatNumber(totals.requests)}>
        {t('dashboard.cards.avg_daily', { value: formatNumber(Math.round(totals.avgDailyRequests || 0)) })}
      </Card>
      <Card title={t('dashboard.cards.quota_used')} value={renderQuota(totals.quota)}>
        {t('dashboard.cards.avg_daily', { value: renderQuota(totals.avgDailyQuotaRaw) })}
      </Card>
      <Card title={t('dashboard.cards.tokens_consumed')} value={formatNumber(totals.tokens)}>
        {t('dashboard.cards.avg_daily', { value: formatNumber(Math.round(totals.avgDailyTokens || 0)) })}
      </Card>
      <Card title={t('dashboard.cards.avg_cost')} value={renderQuota(totals.avgCostPerRequestRaw, 4)}>
        {t('dashboard.cards.tokens_per_request', { value: Math.round(totals.avgTokensPerRequest || 0) })}
      </Card>
    </div>
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg border p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <LeaderCard
          ...
          helper={modelLeaders.mostRequested ? t('dashboard.labels.requests_value', { value: formatNumber(modelLeaders.mostRequested.requests) }) : ''}
        />
        <LeaderCard
          label={t('dashboard.top_models.most_tokens')}
          value={modelLeaders.mostTokens?.model ?? t('dashboard.labels.no_data')}
          helper={modelLeaders.mostTokens ? t('dashboard.labels.tokens_value', { value: formatNumber(modelLeaders.mostTokens.tokens) }) : ''}
        />
        <LeaderCard
          label={t('dashboard.top_models.highest_cost')}
          value={modelLeaders.mostQuota?.model ?? t('dashboard.labels.no_data')}
          helper={modelLeaders.mostQuota ? t('dashboard.labels.quota_consumed', { value: renderQuota(modelLeaders.mostQuota.quota) }) : ''}
        />
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <InsightCard
        ...
        helper={rangeInsights.busiestDay ? t('dashboard.labels.requests_value', { value: formatNumber(rangeInsights.busiestDay.requests) }) : ''}
      />
      <InsightCard
        label={t('dashboard.insights.peak_token_day')}
        value={rangeInsights.tokenHeavyDay?.date ?? t('dashboard.labels.no_data')}
        helper={rangeInsights.tokenHeavyDay ? t('dashboard.labels.tokens_value', { value: formatNumber(rangeInsights.tokenHeavyDay.tokens) }) : ''}
      />
      <InsightCard
        label={t('dashboard.insights.models_in_use')}
        value={formatNumber(totals.uniqueModels)}
        helper={totals.uniqueModels

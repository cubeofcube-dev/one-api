...
import { GradientDefs, chartConfig } from '../services/chartConfig'
import { TFunction } from 'i18next'

interface TrendSparklinesProps {
  t: TFunction
  timeSeries: Array<{ date: string; requests: number; quota: number; tokens: number }>
}
...
interface SparklineCardProps {
  ...
  data: Array<{ date: string; requests: number; quota: number; tokens: number }>
}

const SparklineCard = ({ title, color, dataKey, data }: SparklineCardProps) => (
  <div className="bg-white dark:bg-gray-900 rounded-lg border p-4">
    <h3 className="font-medium mb-4" style={{ color }}>{title}</h3>

          ...
          <div className="mt-4 space-y-4 border rounded-md p-4 bg-muted/5">
            <div className="flex items-center justify-between">
              ...
              {toolEditorDisabled && (
                <span className="text-xs text-destructive">
                  {tr('tooling.editor_disabled', 'Fix JSON error to enable editor')}
                </span>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addToolToWhitelist(customTool, { isCustom: true }))}
              ...
              <div className="flex flex-wrap gap-2 max-h-[100px] overflow-y-auto p-2 border rounded bg-background">
                {availableDefaultTools.map((tool) => {
                  ...
                  return (
                    ...
                    >
                      {tool}
                      {hasPricing && <span className="ml-1 text-[10px] text-blue-500">($)</span>}
                    </Badge>
                  )
                })}
              </div>
